module Poust.Level.Renderer {
    ...
    export function playerEntityRendererFactory(lineWidth: number, strokeStyle: any, bodyFillStyle: any, gunFillStyle): IEntityRenderer {
        ...
        return function (context: CanvasRenderingContext2D, entity: IEntity) {
            ...
            if (ir > 0) {
                ...
                var playerEntity = <Poust.Level.Entity.PlayerEntity>entity;
                ...
                if (state == Poust.Level.Entity.PlayerEntity.STATE_RUNNING && Math.abs(playerEntity._velocityAPX) > 0.0001) {
                ...
                if (state == Poust.Level.Entity.PlayerEntity.STATE_JUMPING) {
                ...
                var armRotation = cycleMultiplier * -Math.PI / 3;
                if (playerEntity._aimAngle != null) {
                    armRotation = -playerEntity._aimAngle;
                }

                context.save();
                context.translate(armShoulderCx, armShoulderCy);
                if (playerEntity._runningLeft) {
                    context.rotate(armRotation + Math.PI / 2);
                } else {
                    context.rotate(-(armRotation + Math.PI / 2));
                }
                var shootingBackward = armRotation < Math.PI / 2 && armRotation > -Math.PI / 2;
